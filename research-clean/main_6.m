...
% Define directory
v1_dir = "D:\RoyStudies\Recordings\hear-my-ship\V1\";
subdirs = dir(v1_dir);
...
for s = 1:length(subdirs)
    dataset_name = subdirs(s).name;
    dataset_dir = fullfile(v1_dir, dataset_name);
    ...
    for i = 1:length(wav_files)
        ...
        try
            ...
            % Ships -> full band [50, 24000] Hz
            [bw, freqs, ~] = process_stability(filepath, info, window_sec, step_sec, [50, 24000], num_peaks); 
    ...
    % Plot Histograms (Between-Files Mean Bandwidths)
    if ~isempty(all_file_mean_bws)
        figure('Name', sprintf('%s - Between-Files BW Fairness', dataset_name), 'Position', [150, 100, 1000, 1000]);
        set(gcf, 'Color', [0.12 0.12 0.12]);
        ...
        for p = 1:num_peaks
            if size(all_file_mean_bws, 2) >= p
                ...
                title(sprintf('%s - Peak %d Between-Files BW (Jain''s Fairness: %.4f)', dataset_name, p, jains_fairness), 'Color', 'w');
