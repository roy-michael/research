...
if ~isempty(left_crossings)
    ...
    idx_below = left_crossings(end);
    if idx_below < peak_local_idx
        idx_above = idx_below + 1;
        diff_below = fft_segment(idx_below) - lower_env(idx_below);
        diff_above = fft_segment(idx_above) - lower_env(idx_above);
        % Linear interpolation to exact zero crossing
        t = -diff_below / (diff_above - diff_below);
        f_exact_L = f_segment(idx_below) + t * (f_segment(idx_above) - f_segment(idx_below));
        mag_exact_L = fft_segment(idx_below) + t * (fft_segment(idx_above) - fft_segment(idx_below));
    else
        f_exact_L = f_segment(idx_below);
        mag_exact_L = fft_segment(idx_below);
    end
else
    [~, l_idx] = min(abs(fft_segment(1:peak_local_idx) - lower_env(1:peak_local_idx)));
    f_exact_L = f_segment(l_idx);
    mag_exact_L = fft_segment(l_idx);
end
...
if ~isempty(right_crossings)
    % Grab the crossing closest to the peak
    idx_below = peak_local_idx + right_crossings(1) - 1;
    if idx_below > peak_local_idx
        idx_above = idx_below - 1;
        diff_above = fft_segment(idx_above) - lower_env(idx_above);
        diff_below = fft_segment(idx_below) - lower_env(idx_below);
        % Linear interpolation to exact zero crossing
        t = -diff_above / (diff_below - diff_above);
        f_exact_R = f_segment(idx_above) + t * (f_segment(idx_below) - f_segment(idx_above));
        mag_exact_R = fft_segment(idx_above) + t * (fft_segment(idx_below) - fft_segment(idx_above));
    else
        f_exact_R = f_segment(idx_below);
        mag_exact_R = fft_segment(idx_below);
    end
else
    [~, temp_idx] = min(abs(fft_segment(peak_local_idx:end) - lower_env(peak_local_idx:end)));
    r_idx = peak_local_idx + temp_idx - 1;
    f_exact_R = f_segment(r_idx);
    mag_exact_R = fft_segment(r_idx);
end

bandwidth = abs(f_exact_R - f_exact_L);
...
plot([f_exact_L, f_exact_R], [mag_exact_L, mag_exact_R], 'ro', 'MarkerSize', 8, 'LineWidth', 2);
