% =========================================================================
% Segmented Multi-Target Acoustic Tracker (Global Aggregation & SNR)
% =========================================================================
clear; clc; close all;

% --- 1. CONFIGURATION ---
dataset_path = 'D:\RoyStudies\Recordings\Croatia\Ocean Sonics\2407_1_600m';

TARGET_FS = 5000; % Safely covers up to 2500 Hz while saving ~90% RAM
f_min_global = 100; % Increased to ignore extreme low-frequency wave/flow noise
f_max_global = 2000;

SEGMENT_SEC = 120; % 2-minute processing blocks
OVERLAP_SEC = 30;  % 30-second overlap
MAX_TARGETS_PER_BLOCK = 10; % Max targets to track locally per segment
MIN_PCT_ACTIVE = 10.0; % Signal must be active for 10% of block to survive (12 seconds)

% --- 2. LOAD AND CONCATENATE ---
clean_dir = dataset_path;
if endsWith(clean_dir, filesep), clean_dir = clean_dir(1:end-1); end
[~, datasetName, ~] = fileparts(clean_dir);

files = dir(fullfile(dataset_path, '*.wav'));
if isempty(files), error('No .wav files found.'); end

% Sort alphanumerically to guarantee chronological order
fileNames = {files.name};
[~, sortIdx] = sort(fileNames);
files = files(sortIdx);

concatenated_signal = [];
global_fs = TARGET_FS;

fprintf('\n========================================\n');
fprintf('Concatenating & Downsampling %d files from: %s\n', length(files), datasetName);

for j = 1:length(files)
file_path = fullfile(dataset_path, files(j).name);
[sig, orig_fs] = audioread(file_path);

% Select Best Channel FIRST
sig = select_best_channel(sig);

% Downsample
if orig_fs > TARGET_FS
    [p, q] = rat(TARGET_FS / orig_fs);
    sig = resample(sig, p, q);
elseif j == 1
    global_fs = orig_fs;
end

concatenated_signal = [concatenated_signal; sig];


end

total_len_sec = length(concatenated_signal) / global_fs;
fprintf('Total recording length: %.2f seconds\n\n', total_len_sec);

% --- 3. SEGMENTED PROCESSING LOOP ---
step_samples = (SEGMENT_SEC - OVERLAP_SEC) * global_fs;
segment_samples = SEGMENT_SEC * global_fs;
num_blocks = floor((length(concatenated_signal) - segment_samples) / step_samples) + 1;

% Database to hold all valid targets found across all blocks
global_target_db = [];

for b = 1:num_blocks
start_idx = 1 + (b-1) * step_samples;
end_idx = start_idx + segment_samples - 1;
start_time_sec = start_idx / global_fs;

fprintf('=== Block %d / %d (%.1f to %.1f sec) ===\n', b, num_blocks, start_time_sec, end_idx/global_fs);

segment_sig = concatenated_signal(start_idx:end_idx);

% Compute STFT for this block
[P, F, T] = compute_spectrogram(segment_sig, global_fs);

% Find peaks using adaptive SNR & Fractional Harmonic Filter
[target_freqs, target_powers, num_targets] = find_fundamental_targets(P, F, f_min_global, f_max_global);

if num_targets > 0
    if num_targets > MAX_TARGETS_PER_BLOCK
        [~, sort_idx] = sort(target_powers, 'descend');
        target_freqs = target_freqs(sort_idx(1:MAX_TARGETS_PER_BLOCK));
        target_powers = target_powers(sort_idx(1:MAX_TARGETS_PER_BLOCK));
        num_targets = MAX_TARGETS_PER_BLOCK;
    end
    
    max_power = max(target_powers);
    rel_intensities = target_powers - max_power;
    
    % Extract features and filter out transient noise spikes
    valid_block_targets = track_and_extract_features(P, F, T, target_freqs, rel_intensities, MIN_PCT_ACTIVE);
    
    if ~isempty(valid_block_targets)
        % Append to global database [Freq, BW, df, fm, Rel_Int]
        global_target_db = [global_target_db; valid_block_targets];
    end
else
    fprintf('  No targets found.\n');
end
fprintf('\n');


end

% --- 4. GLOBAL AGGREGATION & REPORTING ---
fprintf('\n========================================\n');
fprintf('AGGREGATING GLOBAL TARGETS...\n');

% Group targets that are within 5 Hz of each other
cluster_tolerance = 5.0;
final_targets = [];
% final_targets structure: [Avg_Freq, Block_Count, Max_BW, Max_df, Avg_fm, Max_Int]

while ~isempty(global_target_db)
base_f = global_target_db(1, 1);

% Find all targets in database within tolerance of base_f
idx = find(abs(global_target_db(:,1) - base_f) <= cluster_tolerance);

cluster = global_target_db(idx, :);

% Aggregate data for this cluster
avg_f = mean(cluster(:, 1));
block_count = size(cluster, 1);
max_bw = max(cluster(:, 2));
max_df = max(cluster(:, 3));
avg_fm = mean(cluster(:, 4));
max_int = max(cluster(:, 5)); % Highest relative intensity achieved

final_targets = [final_targets; avg_f, block_count, max_bw, max_df, avg_fm, max_int];

% Remove these targets from the pool
global_target_db(idx, :) = [];


end

% Sort by Survivability (Block Count) and keep Top 10
if ~isempty(final_targets)
[~, sort_idx] = sort(final_targets(:, 2), 'descend');
final_targets = final_targets(sort_idx, :);

num_final = min(10, size(final_targets, 1));
final_targets = final_targets(1:num_final, :);

% Recalculate Global Intensity (Loudest of the Top 10 is 0 dB)
global_max_int = max(final_targets(:, 6));

fprintf('\nFINAL REPORT: TOP %d UNIQUE AGGREGATED TARGETS\n', num_final);
for i = 1:num_final
    f = final_targets(i, 1);
    pct_survived = (final_targets(i, 2) / num_blocks) * 100;
    bw = final_targets(i, 3);
    df = final_targets(i, 4);
    fm = final_targets(i, 5);
    int_db = final_targets(i, 6) - global_max_int;
    
    fprintf('Target %d: ~%.2f Hz\n', i, f);
    fprintf('  Active Time      : %5.1f%% of total recording\n', pct_survived);
    fprintf('  Peak Intensity   : %5.2f dB (relative to top targets)\n', int_db);
    fprintf('  Max Carson BW    : %5.2f Hz (Max df: %.2f, Avg fm: %.2f)\n\n', bw, df, fm);
end


else
fprintf('No sustained targets found in the entire dataset.\n');
end

% --- 5. GLOBAL VISUALIZATION ---
fprintf('Rendering Global Annotated Spectrogram...\n');
figure('Name', 'Global Annotated Spectrogram', 'NumberTitle', 'off', 'Position', [100, 100, 1200, 600]);

% Create a render-safe, coarse spectrogram for the whole file
window_size_global = round(global_fs * 1.5); % 1.5 sec windows ~ limits cols to ~3000
noverlap_global = round(window_size_global * 0.5);
nfft_global = 8192; % Standard resolution for visual overview

[S_glob, F_glob, T_glob] = spectrogram(concatenated_signal, window_size_global, noverlap_global, nfft_global, global_fs);
P_glob_dB = 10 * log10(abs(S_glob).^2);

% Crop to bounds
plot_idx = find(F_glob >= f_min_global & F_glob <= f_max_global);
imagesc(T_glob, F_glob(plot_idx), P_glob_dB(plot_idx, :));
axis xy; axis tight; colormap(jet);
ylabel('Frequency (Hz)', 'FontWeight', 'bold');
xlabel('Time (s)', 'FontWeight', 'bold');
title(sprintf('Global Spectrogram: %s (Top %d Targets)', datasetName, size(final_targets,1)), 'Interpreter', 'none');
colorbar;

% Overlay Top 10 Targets
hold on;
for i = 1:size(final_targets, 1)
target_freq = final_targets(i, 1);
% Draw dashed white line
yline(target_freq, 'W--', 'LineWidth', 1.5, 'Alpha', 0.8);
% Add text label at the far right
text(max(T_glob) - (max(T_glob)*0.02), target_freq + 25, sprintf('T%d', i), ...
'Color', 'white', 'FontWeight', 'bold', 'FontSize', 10);
end
hold off;

fprintf('Processing Complete.\n');

% =========================================================================
%                         LOCAL FUNCTIONS
% =========================================================================

function sig_mono = select_best_channel(sig_matrix)
if size(sig_matrix, 2) > 1
channel_power = rms(sig_matrix, 1);
[~, best_idx] = max(channel_power);
sig_mono = sig_matrix(:, best_idx);
else
sig_mono = sig_matrix;
end
end

function [P, F, T] = compute_spectrogram(signal, fs)
window_size = round(fs * 0.25); % 0.25 seconds
noverlap = round(window_size * 0.5); % 50% overlap prevents memory crash
nfft = 65536; % Maintains extreme sub-Hz resolution
[S, F, T] = spectrogram(signal, window_size, noverlap, nfft, fs);
P = abs(S).^2;
end

function [fundamentals, fund_powers, num_targets] = find_fundamental_targets(P, F, f_min, f_max)
% 85th percentile ignores short bursts but preserves sustained tonal lines
composite_P = prctile(P, 85, 2);
composite_P_dB = 10 * log10(composite_P);

valid_idx = find(F >= f_min & F <= f_max);
F_valid = F(valid_idx);
P_valid_dB = composite_P_dB(valid_idx);

% Adaptive Background Normalization (SNR)
% A 100-bin window is roughly 7.5 Hz wide at nfft=65536
bg_profile = medfilt1(P_valid_dB, 100); 
snr_dB = P_valid_dB - bg_profile;

min_dist_bins = round(15 / (F(2) - F(1))); 

% Find peaks based on SNR, completely immune to global loud noise floors
[peaks_snr, peak_locs] = findpeaks(snr_dB, ...
    'MinPeakProminence', 3.0, ...  % Must be 3dB above the local noise floor
    'MinPeakDistance', min_dist_bins);
    
raw_freqs = F_valid(peak_locs);
peaks_absolute_dB = P_valid_dB(peak_locs); % Get real power for later

fundamentals = [];
fund_powers = [];

% Sort by SNR (strongest relative peaks first)
[peaks_snr, sort_idx] = sort(peaks_snr, 'descend');
raw_freqs = raw_freqs(sort_idx);
peaks_absolute_dB = peaks_absolute_dB(sort_idx);

% Aggressive "Ghost Base" Harmonic Filter
max_harmonic_order = 12;
tolerance = 4.0; % Allowed drift/smear in Hz

for i = 1:length(raw_freqs)
    curr_f = raw_freqs(i);
    curr_db = peaks_absolute_dB(i);
    is_harmonic = false;
    
    for j = 1:length(fundamentals)
        fund = fundamentals(j);
        
        % Test if 'fund' is the 1st, 2nd, 3rd... up to 12th harmonic of a ghost base
        for m = 1:max_harmonic_order
            base_f = fund / m;
            if base_f < 15, break; end 
            
            n_ratio = curr_f / base_f;
            n_int = round(n_ratio);
            
            % If curr_f is a clean multiple of this ghost base, it's a harmonic
            if abs(n_ratio - n_int) * base_f < tolerance
                is_harmonic = true;
                ratio_str = sprintf('%.2fx', n_int / m);
                fprintf('  -> Discarded %.2f Hz (Identified as %s harmonic of %.2f Hz)\n', curr_f, ratio_str, fund);
                break;
            end
        end
        if is_harmonic, break; end
    end
    
    if ~is_harmonic
        fundamentals = [fundamentals, curr_f];
        fund_powers = [fund_powers, curr_db];
    end
end
num_targets = length(fundamentals);


end

function valid_targets = track_and_extract_features(P, F, T, target_freqs, rel_intensities, min_pct)
track_margin = 30;
dt = T(2) - T(1);
track_fs = 1 / dt;
valid_targets = [];

for i = 1:length(target_freqs)
    target_f = target_freqs(i);
    track_idx = find(F >= (target_f - track_margin) & F <= (target_f + track_margin));
    
    [track_power, local_max_idx] = max(P(track_idx, :), [], 1); 
    global_max_idx = track_idx(local_max_idx);
    raw_track = F(global_max_idx);
    
    track_power_dB = 10 * log10(track_power);
    p95_dB = prctile(track_power_dB, 95); 
    p10_dB = prctile(track_power_dB, 10); 
    
    if (p95_dB - p10_dB) < 3.0
        pct_active = 100.0;
    else
        active_thresh_dB = p10_dB + ((p95_dB - p10_dB) * 0.4); 
        pct_active = (sum(track_power_dB > active_thresh_dB) / length(T)) * 100;
    end
    
    % SUSTAINED ACTIVITY CHECK
    if pct_active < min_pct
        fprintf('  [Ignored] %.2f Hz (Transient noise: Active only %.1f%%)\n', target_f, pct_active);
        continue;
    end
    
    smooth_track = medfilt1(raw_track, 50);
    center_f = median(smooth_track);
    delta_f = prctile(abs(smooth_track - center_f), 95);
    
    mod_signal = smooth_track - center_f;
    N = length(mod_signal);
    mod_fft = abs(fft(mod_signal));
    half_idx = floor(N/2);
    mod_fft = mod_fft(1:half_idx);
    mod_freqs = (0:half_idx-1) * (track_fs / N);
    
    [~, mod_peak_idx] = max(mod_fft(2:end));
    f_m = mod_freqs(mod_peak_idx + 1);
    
    carson_bw = 2 * (delta_f + f_m);
    
    fprintf('  [Target %d] Base: %6.2f Hz | BW: %6.2f Hz (df: %5.2f, fm: %.2f) | Int: %6.2f dB\n', ...
        size(valid_targets,1)+1, center_f, carson_bw, delta_f, f_m, rel_intensities(i));
        
    valid_targets = [valid_targets; center_f, carson_bw, delta_f, f_m, rel_intensities(i)];
end


end