...
% The allowed +/- frequency drift when checking harmonics
tolerance = 5.0; 
...
for i = 1:length(raw_freqs)
    ...
    for j = 1:length(fundamentals)
        ...
        % --- ENHANCED HARMONIC CHECK ---
        % Check integer harmonics (x1, x2, x3)
        int_ratio = round(ratio);
        
        % Check half-integer harmonics common in engines (x1.5, x2.5)
        half_ratio = round(ratio * 2) / 2;
        
        % Check 1/3 harmonics (x1.33, x1.66)
        third_ratio = round(ratio * 3) / 3;

        % Determine which fractional ratio the signal is closest to
        [min_diff, best_match] = min([abs(ratio - int_ratio), abs(ratio - half_ratio), abs(ratio - third_ratio)]);
        
        % Check if the frequency difference is within the physical tolerance
        if (min_diff * fund) < tolerance
            % Verify amplitude: a harmonic shouldn't be drastically louder than the fundamental
            if curr_db < fund_db(1) + 2.0 % Allow a slight margin in case the fundamental faded
                is_harmonic = true;
