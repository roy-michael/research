...
This section establishes the mathematical and physical foundations required to understand how ocean environments distort acoustic signals. By defining the underlying mechanisms of signal spreading, we create a baseline for analyzing specific source geometries.
...
A surface vessel represents the most extreme case of acoustic smearing. This section analyzes this geometry to demonstrate the worst-case scenario, where the acoustic source is physically coupled to the dynamic ocean boundary, resulting in maximal delay and Doppler spread.
...
This subsection calculates the geometry of the reflection to prove that for a surface source and distant receiver, the acoustic path reflects at an extremely shallow angle, forcing the direct and surface-scattered paths to overlap almost perfectly.
...
This subsection models the physical motion of the vessel to prove that its vertical heave inherently transforms the emitted acoustic wave into a Phase/Frequency Modulated (FM) signal before it even propagates into the channel.
...
This subsection transitions the time-domain phase modulation into the frequency domain to mathematically prove that the vessel's physical heave expands the single tonal into an infinite series of frequency sidebands.
...
The time-domain pressure wave $p_{tx}(t)$ as the real part of a complex analytic signal:
\[ p_{tx}(t) = P_0 \text{Re} \left\{ e^{i 2\pi f_0 t} \cdot e^{i \beta \sin(2\pi f_w t)} \right\} \]

After applying the Jacobi-Anger identity, which expands the exponential of a sine function into an infinite series of Bessel-weighted harmonics:
...
This subsection models the reflection off the ocean surface to show how the moving waves act as a dynamic scatterer, imposing a time-varying phase shift (Doppler) that constantly changes the reflected path.
...
\end{align}
This formulation is the standard Kirchhoff approximation for rough surface scattering. The negative sign represents the $180^\circ$ phase inversion at the pressure-release air-water boundary. $R_0$ is the amplitude loss due to rough-surface scattering (energy lost from the specular direction). The exponential term represents the additional phase shift (Doppler) caused by the extra geometric distance the wave must travel as the surface moves vertically.

\subsubsection{The Smeared Received Signal}
This subsection combines the modulated source signal and the time-varying channel interaction into a single final equation, proving mathematically why the received signal suffers from continuous, multiplicative amplitude fading and phase distortion.
...
This subsection links the derived geometry and physics directly to standard acoustic channel theory, confirming that this specific environment strictly satisfies the formal criteria for both Delay and Doppler spreading.
...
To highlight the impact of surface coupling, this section contrasts the surface vessel with a deeply submerged source. By isolating the source from the dynamic boundary, we can demonstrate how physical decoupling dramatically stabilizes the acoustic signal.
...
This subsection proves mathematically that placing the source below the wave base eliminates the physical heave, returning the transmitted signal to a pure, unmodulated tone.
...
This subsection shows how the depth of the source geometrically separates the direct path from the surface reflection, creating a macroscopic time delay that breaks the multiplicative interference.
...
This subsection presents the final received signal equation for an underwater source, proving that the primary acoustic signal arrives completely pristine and is structurally decoupled from surface dynamics.
...
Transitioning from theoretical physics to practical application, this section outlines the signal processing architecture. The flow diagram visualizes the algorithm designed to process, analyze, and mitigate the acoustic smearing effects established in the previous sections.

\resizebox{!}{0.85\textheight}{\begin{tikzpicture}[
    >=Stealth,
...
\node (c3) [purplebox, below=of c2] {2c. CALCULATE BANDWIDTH\\PARAMETERS $H_{i,j}$ ($j=1..J$)};
...
\node (matrix) [bluebox, below=1.5cm of loopgroup] {BANDWIDTH PARAMETER MATRIX $H_{i,j}$\\Data for $I$ frequencies, $J$ buffers};
