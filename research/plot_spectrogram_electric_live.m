% MAIN_LIVE_SPECTROGRAM_ELECTRIC
% Reads all audio files in both Electric datasets (Deep and Shallow) and plots 
% full-resolution live spectrograms for comparison.

% Define paths for both datasets
DATASETS = {
    'C:\Users\Roy\Recordings\Garda_2_26\2_Deep Water\Electric', 'Deep Water';
    'C:\Users\Roy\Recordings\Garda_2_26\1_Shallow Water\Electric', 'Shallow Water'
};
...
for d = 1:size(DATASETS, 1)
    ...
    fprintf('\n======================================================\n');
    fprintf('Processing Dataset: Electric (%s)\n', DATASET_NAME);
    fprintf('======================================================\n');
...
function plot_spectrogram(samplerate, data, dataset_name, window_offset)
    ...
    % The Electric dataset creates a massive matrix which can crash MATLAB's renderer.
    ...
    fig = figure('Name', sprintf('Live Spectrogram of Electric (%s)', dataset_name), ...
                 'Position', [100 + (window_offset-1)*50, 400 - (window_offset-1)*50, 1200, 800]);
    ax = axes(fig);
    ...
    xlabel(ax, 'Time [sec]');
    title(ax, sprintf('High-Resolution Spectrogram of Electric (%s)', dataset_name));
