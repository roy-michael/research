% plot_garda_electric_envelope.m
% Reads the entire run of Garda Electric recordings, removes the DC offset,
...
try
    ...
    fig = figure('Name', 'Garda Electric Amplitude Envelopes (DC Removed)', 'Position', [100, 100, 1400, 700]);
    ax = axes('Parent', fig);
    hold(ax, 'on');
    ...
    % Plot upper envelope in vibrant blue
    plot(ax, t_ds, env_up_ds, 'Color', [0.0000 0.4470 0.7410], 'LineWidth', 1.5, 'DisplayName', 'Upper Envelope');
    
    % Plot lower envelope in warm red
    plot(ax, t_ds, env_lo_ds, 'Color', [0.8500 0.3250 0.0980], 'LineWidth', 1.5, 'DisplayName', 'Lower Envelope');
    ...
    title(ax, 'Garda Electric Recording - Signal Envelopes (DC Removed, Time Domain)');
