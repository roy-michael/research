% plot_welch_demon.m
% Computes and compares Welch PSD and DEMON spectra for Electric, Petrol,
...
DATASETS = {
    ...
    'D:\RoyStudies\Recordings\Garda_2_26\2_Deep Water\Electric', 'Electric (Deep Water)';
    'D:\RoyStudies\Recordings\Croatia\Ocean Sonics\2407_1_600m', 'Dataset 2407_1'
};
...
fid = fopen(fullfile(output_dir, 'spectral_data.json'), 'w');
fprintf(fid, '%s', json_str);
fclose(fid);
fprintf('Saved spectral_data.json to output directory\n');
...
colors = [
    ...
    0.4660 0.6740 0.1880  % Green (Dataset 2407_1)
];

% --- Figure 1: Welch PSD ---
fig_welch = figure('Name', 'Welch PSD (Signals Highlighted)', 'Position', [100, 100, 1200, 700], 'Visible', 'off');
...
title(ax_welch, 'Welch PSD: Highlighted Signals & Bandwidths (0-4 kHz)');
grid(ax_welch, 'on');
xlim(ax_welch, [0, 4]);
...
title(ax_demon, 'DEMON Spectrum: Highlighted Signals & Bandwidths (0-4 kHz)');
grid(ax_demon, 'on');
