% plot_croatia_bandwidth_measurement.m
% Computes the Narrowband Welch PSD and its local moving average envelope for the Croatia snake recording,
...
try
    ...
    fig = figure('Name', 'Croatia Bandwidth Boundaries via Envelope Intersections', 'Position', [200, 100, 1400, 800]);
    ax = axes('Parent', fig);
    hold(ax, 'on');
    
    % Plot original PSD (thin line, purple styled for Croatia)
    plot(ax, f_nb, psd_nb_db, 'Color', [0.4940 0.1840 0.5560], 'LineWidth', 1.0, 'DisplayName', 'Welch PSD');
    
    % Plot envelope (dashed yellow-orange line)
    ...
    title(ax, 'Croatia snake Welch PSD & Moving Average Envelope with Intersection Boundaries (50 Hz - 4 kHz)');
    legend(ax, 'Location', 'northeast');
    
    % Enable zoom
    zoom(fig, 'on');
    
    fprintf('Full range Croatia bandwidth boundary plot is live!\n');
