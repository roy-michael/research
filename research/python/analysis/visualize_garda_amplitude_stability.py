...
def load_and_process_dataset(dataset_dir, dataset_name):
    print(f"Loading Garda {dataset_name} in {dataset_dir}...")
    ...
    for f in files[:20]: # Limit to first 20 files for performance
        try:
            ...
            # Filter from 20 to 1000 Hz to cover Garda target harmonics
            filtered = butter_bandpass_filter(data, 20.0, 1000.0, fs, order=5)
...
def main():
    ...
    datasets = {
        'Shallow_Petrol': r"D:\RoyStudies\Recordings\Garda_2_26\1_Shallow Water\Petrol",
        'Deep_Petrol': r"D:\RoyStudies\Recordings\Garda_2_26\2_Deep Water\Petrol",
        'Shallow_Electric': r"D:\RoyStudies\Recordings\Garda_2_26\1_Shallow Water\Electric",
        'Deep_Electric': r"D:\RoyStudies\Recordings\Garda_2_26\2_Deep Water\Electric"
    }
    
    target_freqs = [45.0, 90.0, 135.0, 180.0, 225.0]
    ...
    for ds_name, ds_dir in datasets.items():
        ...
        plt.title(f"Option 1: Garda Amplitude Spread (dB) Violin Plot\nDataset: {ds_name}", fontsize=14, fontweight='bold')
        plt.xlabel("Harmonic Frequency", fontsize=12)
        plt.ylabel("Amplitude (dB)", fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        violin_path = os.path.join(output_dir, f"garda_stability_option1_violin_{ds_name}.png")
        ...
        plt.title(f"Option 2: Garda Amplitude Time-Series with Rolling Std Envelope\nDataset: {ds_name} (Window={window})", fontsize=14, fontweight='bold')
        plt.xlabel("Time (seconds)", fontsize=12)
        plt.ylabel("Amplitude (dB)", fontsize=12)
        plt.legend(loc='upper right')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        ts_path = os.path.join(output_dir, f"garda_stability_option2_timeseries_{ds_name}.png")
        plt.savefig(ts_path, dpi=300)
        plt.close()
        
        print(f"Generated Option 1 and Option 2 plots for Garda {ds_name}.")
    ...
    if cv_data:
        ...
        plt.title("Option 3: Garda Amplitude Coefficient of Variation (CV) Comparison\nLower CV = Higher Stability", fontsize=14, fontweight='bold')
        plt.xlabel("Dataset", fontsize=12)
        plt.ylabel("Coefficient of Variation (std / mean)", fontsize=12)
        plt.grid(True, axis='y', alpha=0.3)
        plt.tight_layout()
        cv_path = os.path.join(output_dir, "garda_stability_option3_cv_comparison.png")
