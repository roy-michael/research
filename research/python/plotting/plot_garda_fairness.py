...
def extract_freq_amp_garda(file_paths, name, target_freq=45.0, segment_length_seconds=2):
    min_freq = max(10.0, target_freq - 20.0)
    max_freq = target_freq + 20.0
    f, a, _ = extract_frequencies_for_files(file_paths, name, min_freq, max_freq, segment_length_seconds)
    return f, a

def main():
    directory = r"C:\Users\Roy\Recordings\Garda_2_26\1_Shallow Water\Petrol"
    ...
    for i, h in enumerate(harmonics):
        ...
        # 1. Phase Fairness
        p_phases = extract_pll_phases(wav_files, f"garda_H{h}", target_f, SEGMENT_SEC, bandpass_margin=20.0)
        ...
        # 2. Freq/Amp Fairness
        freqs, amps = extract_freq_amp_garda(wav_files, f"Garda_H{h}_FA", target_f, SEGMENT_SEC)
    ...
    fig_p.tight_layout()
    fig_p.savefig('garda_pll_harmonic_fairness.png')
    
    fig_f.tight_layout()
    fig_f.savefig('garda_jains_freq_harmonics.png')
    
    fig_a.tight_layout()
    fig_a.savefig('garda_jains_amp_harmonics.png')
    
    print("Saved all Garda fairness plots!")
